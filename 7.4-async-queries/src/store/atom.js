...
export const notifications = atom({
  ...
  default: { network: parseInt(Math.random() * 100), jobs: parseInt(Math.random() * 100), messaging: parseInt(Math.random() * 100), notifications: parseInt(Math.random() * 100) }, // Default static value for notification counts
});
...
export const notificationsSelector = selector({
  ...
  get: async () => {
    try {
      const response = await axios.get("https://mocki.io/v1/8a4d6482-c701-4525-8742-03c39d2ff03c"); // Fetch notifications from the server
